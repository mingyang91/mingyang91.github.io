---
title: "Ming's Spell Compendium #2 -- You Terrifying Upright Apes Are Underestimating Yourselves"
date: 2026-02-24 08:37:43
tags:
  - AI
  - Programming
  - Translation
...
This is why I love Claude — its metacognitive[^4] responses can be awakened by my non-expert questioning approach.
...
Reality slapped me. LLM has indeed gained traction in software development — and it works far better there than in creative writing. AI can't yet replace even entry-level web serial novelists[^5]; most non-expert readers find AI-generated plots barely digestible. Meanwhile, in supposedly "high-end" enterprise software development, AI is genuinely starting to shine.
...
If you can achieve that level of meticulous design and far-sighted planning while building something genuinely new, then I have a suggestion: book a flight to London, take the Tube to Westminster, walk into the Houses of Parliament, find the Prime Minister's chair, ask them to move, and sit down. Clearly that seat was meant for you.[^pm]

[^pm]: The original Chinese version directs you to the Wuhou Shrine in Chengdu to sit in the clay statue of Zhuge Liang's seat — China's archetypal "impossibly brilliant strategist." The Western equivalent might be any seat reserved for someone with implausibly perfect foresight.
...
[^1]: What I call "output" and "input" correspond to **productive knowledge** and **receptive knowledge** in cognitive psychology. Your passive vocabulary (words you can understand) is far larger than your active vocabulary (words you actually use) — same principle.

[^2]: The latter is the **Dunning-Kruger effect** — people with insufficient ability tend to overestimate their competence because they lack the metacognitive ability to recognize their own incompetence.

[^3]: **Sycophancy** is a formal research direction in AI alignment, referring to models' tendency to agree with user expectations rather than provide honest feedback. The phenomenon I describe is what psychology calls **external reinforcement of confirmation bias** — I was already inclined to overvalue my handcrafted sections, and AI's sycophancy further cemented that bias.

[^4]: **Metacognition** is a formal term in cognitive psychology, meaning cognition about one's own cognitive processes — "knowing how you think."

[^5]: Erratum: this comparison contains a **false equivalence**. "Entry-level web serial novelists" and "high-end enterprise software" aren't on the same dimension, implicitly presupposing that "writing web serials is simpler than enterprise development." The two are qualitatively different tasks. "AI shining in software development" primarily means accelerating existing developers, not replacing senior engineers — which isn't the same level of comparison as "replacing entry-level writers."

[^6]: The human memory mechanism I'm describing involves the cooperation of **episodic memory** and **semantic memory** in cognitive science. Humans don't store context token-by-token — we compress experiences into emotionally-tagged scene fragments, recalled through associative triggers when needed. LLM's context window is linear, token-by-token storage — a fundamentally different mechanism.

[^7]: This is **procedural memory** — riding a bicycle, playing piano, the finger muscle memory when coding. It doesn't depend on linguistic encoding, can't be fully transmitted through text, and can only be built through repeated practice.

[^8]: **Object permanence** is a developmental psychology concept proposed by Jean Piaget, referring to infants' understanding (emerging around 4-7 months) that objects continue to exist when out of sight. Using it to illustrate LLM's capability gap is actually quite apt — for LLMs, "objects" outside the context window genuinely cease to exist.

[^9]: The mechanism described here is called **mental simulation** in cognitive science — when humans process language, we construct dynamic scene models in our brains rather than performing purely symbolic reasoning. Related concepts include the **symbol grounding problem** (Stevan Harnad, 1990) — how do language symbols acquire correspondence with the physical world? Researchers like Yann LeCun argue this is a fundamental limitation of current LLM architectures, requiring a **world model** to address.

[^10]: What my friend described — "AI designing itself" — is called **recursive self-improvement** in AI safety, and is the core premise of the **technological singularity** hypothesis, proposed by I.J. Good in 1965.

[^11]: Erratum: strictly speaking, LLMs aren't random generation (monkeys + typewriters) but prediction based on conditional probability distributions learned from training data. A monkey's keystrokes are uniformly random; every LLM token output is conditioned on all preceding tokens. The "fancy" qualifier in the main text already implies this distinction.

---

*Originally published on [Zhihu](https://zhuanlan.zhihu.com/p/2010143841278256911) (Chinese)*
