...
<!-- TODO: 这里需要补充更直接的调试难度指标，例如：调试轮次数、agent thinking 中尝试方案的次数、测试回归出现频次。token 消耗是间接指标，不能直接等同于调试难度。 -->

我观察到：5000+ 行上帝函数的屎山组，调试成本（轮次/token）约等于 QG 组，有时甚至略低。

其中 scheme interpreter 一直是难度最大的，也是最不利于我严格规则的场景。主办方是我，规则是我定的，裁判是我，参赛的也是我——我想让我自己赢，竟然费了这么大周折。
...
覆盖 11 轮实验 (R21-R32)，5 种语言 (Rust/Go/Java/TypeScript/Scala)，共 111 次完整运行。自 R21 起，测试级别（27 级）、QG 规则、编排方法均已冻结，后续轮次仅修复调度器和统计工具的 bug，不影响实验数据本身。
...
但值得注意的是：最后几轮数据观察下来，QG 组已经可以在 27 级测试中与屎山组成本打平，甚至略低于屎山组了。有软件工程经验的人大概能意识到，一旦 QG 组成本与屎山组交叉，那么后面的走势屎山组将迎来维护成本爆炸，绝无可能扳回胜局。
...
<!-- 审稿提醒：不可证伪——"屎山惩罚一定会到来，只是测试不够长"使论点无法被反驳。考虑改为更中性的表述，如"在本实验的 27 级范围内未观察到屎山惩罚拐点，更长周期的实验可能会得出不同结论"。 -->
...
2. **人类标准是 AI 的枷锁。** 实验观察：人类规范让 agent 输掉 17 轮，屎山惩罚在 27 级范围内未到来。
