...
<!-- 审稿提醒：假二分法——这里只对比了"完全不还债"和"用人类标准每轮还债"两组，缺少 AI 原生 QG 组和周期性还债组。当前结论只能说明"这套特定的人类 QG 规则在此实验中拖累了 AI"，推广为一般性结论需要更多对照组数据。考虑在正式发表前补充，或在行文中更明确地限定结论范围。 -->
这就是赛博田忌赛马——屎山组用"下等马"（不守规范的代码）赢了 QG 组的"上等马"（严格规范的代码）。赢的不是马，是赛制。人类标准的赛制对 AI 来说，本身就是一种惩罚。

<!-- 审稿提醒：不可证伪——"屎山惩罚一定会到来，只是测试不够长"使论点无法被反驳。考虑改为更中性的表述，如"在本实验的 28 级范围内未观察到屎山惩罚拐点，更长周期的实验可能会得出不同结论"。 -->
...
1. **屎山组**：不还债（已有数据，完赛）
2. **人类 QG 组**：每轮按人类规范还债（已有数据，未完赛）
3. **AI 原生 QG 组**：只约束模块边界和接口，内部自由（缺失）
4. **周期性还债组**：平时自由，每 N 轮集中重构（缺失）

目前有 1 和 2 的数据。但即便如此，我观察到的趋势已经很明确：**在本实验中，人类标准对 AI 是枷锁。**
...
<!-- 审稿提醒：过度泛化——实验任务是 Scheme 解释器（纯计算、无 IO、无并发、无外部依赖），从中得出的数字直接推广到 CRUD 后端开发需要额外论证。建议明确标注这是跨领域的个人实践经验，而非本实验的直接结论。 -->
...
上面的两列分法是粗略的。R21 之前的早期实验（4 个惩罚级别中仅 1 个生效，debt_score=1.64）让我总结出更精确的分类：
...
**什么不是 AI 的技术债：** 函数 500 行、嵌套 6 层、用 `for` 而非 `.iter().map()`、变量名叫 `x`、缺注释——这些都是人类可读性问题，对 AI 的修改成本为零。AI 的技术债用编译错误数、需手动修补的站点数、不相关模块的测试失败数来衡量，不用 style linter 的警告数来衡量。

这也解释了为什么我设计的惩罚关卡大部分没有生效：它们是纯增量任务（加一个 match arm），不需要架构重写。唯一生效的那个（case-lambda）恰好强制了类型级联变更。

### 关于嵌套深度

你的记录提到 nesting limit=3 对解释器来说太残酷了——`eval → match → match → arm logic` 三层 match 是最低限度，任何错误处理就会违规。这个观察完全正确。

当前 QG 配置已经放宽到 **nesting-threshold=6, function-limit=300 行**。从 R28-R32 的 QG 运行数据看，friction（agent 在 thinking 中提到 nesting/clippy/refactor 的次数）在 L18 平均 3.4 次，在 L17 平均 1.8 次——说明即便放宽到 6 层，难度墙级别的结构性嵌套仍然会触发摩擦。但这个摩擦是可控的，不再像早期 nesting=3 时那样直接打死 agent。
...
<!-- 审稿提醒：类比的局限性——四色定理是数学证明，验证是确定性的、完备的；AlphaGo 是完美信息博弈，有明确胜负判定。软件正确性面临状态空间爆炸、环境依赖、非功能需求（安全、性能、UX）等，比数学证明和围棋复杂得多。这些类比支持"可验证性比可读性重要"的方向性论点，但不能直接推导出"软件也能这样做"的具体结论。考虑在正文中补充一句对类比局限性的承认。 -->
...
<!-- 审稿提醒：结论范围——实验证据严格来说只能支持"在 Scheme 解释器任务上，用特定的人类 lint 规则约束 Claude/Codex 会降低完赛率"。以下结论中"推翻重建"、"枷锁"等表述超出了单一实验的支撑范围。部分结论（如分层建议、务实原则）更多来自个人实践经验而非本实验数据。考虑在发表前更明确地区分"实验直接支持的结论"和"基于实验启发的个人观点"。 -->
# 结论
