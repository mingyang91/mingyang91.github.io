...
**第一，信任类型体操的防护。** 前面几节我们花了大量精力把约束编码到类型系统里——`opaque type` 防混淆、`sealed trait` 穷举错误、`NonEmptyList` 防空。既然已经在类型层面投入了这些成本，就应该信任编译器能守住这些防线，**不需要在运行时再加一层防御性检查**。绝对化表述的第一层意思是：编译器已经在守门了，你不需要在门后面再修一堵墙。
...
这个偏差在实践中反复出现。我最常纠正的一个行为：agent 遇到一个返回 `Either[AppError, T]` 的函数，为了让当前代码编译通过，会默认加一个 `.getOrElse(someDefault)` 把错误静默吞掉。来自配置文件的数据——吞掉。来自数据库的查询结果——吞掉。来自用户提交的输入——吞掉。每一个被吞掉的错误都是一个藏在地毯下的 bug，直到用户问"我的数据去哪了"才被发现。

所以规则文件里写的是：**除非业务场景明确要求 Option 的默认值，否则禁止用 `.getOrElse` 静默吞掉 `Either` 的 Left。** 这条规则从字面上看是"绝对禁止"，但它的真实意思是：把默认行为从"吞掉错误"翻转为"传播错误"，只有在人类显式决定"这里确实应该用默认值"时才例外。
