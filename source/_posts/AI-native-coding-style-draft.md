...
读过我的规则文件的人可能会注意到大量绝对化的断言——"编译器报错必须修复，不许绕过"、"类型系统的判断是最终裁决"、"编译通过即正确"。严格来说，这些并不总是对的：编译器有 bug，类型系统有表达力盲区，编译通过不代表业务逻辑正确。

但这是**故意的**。

编码模型的训练数据里包含了海量的"现实世界代码"——而现实世界代码的主体是什么？是用 `@SuppressWarnings` 压掉编译器警告、用 `.asInstanceOf` 绕过类型检查、用 `// noinspection` 让 IDE 闭嘴的权宜之计。模型在训练阶段见过太多"编译器说不行，但我有办法绕过去"的成功案例。结果就是：**模型的默认倾向是把编译器当作可以协商的建议者，而不是必须遵守的裁判。**

绝对化表述的作用不是描述真理，而是**对抗这种训练偏差**。当规则文件说"编译器的判断是最终裁决"，它的真实意思不是"编译器永远正确"，而是"在你想绕过编译器之前，先假设是你写错了"。这是一个反方向的力，把模型从"默认寻找绕过方式"拉回到"默认信任工具链、修正自己的代码"。就像矫正视力的镜片——镜片本身是"歪"的，但戴上之后看到的世界是正的。

这也意味着：**规则文件的绝对化程度应该随模型能力变化而调整。** 如果未来的模型训练数据质量提升，或者 RLHF 对齐策略改变了默认偏好，模型不再倾向于绕过编译器，那"绝对信任编译器"的规则就可以放松为"优先信任编译器"甚至移除。规则文件不是宪法，它是针对特定模型版本的校准参数。
