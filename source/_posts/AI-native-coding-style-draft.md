...
读过我的规则文件的人可能会注意到：里面大量使用"**禁止**"、"**必须**"、"**永远不要**"这类绝对化措辞。严格来说，这些绝对化表述并不总是正确的——现实中几乎任何规则都有例外。

但这是**故意的**。

编码模型的训练数据里包含了海量的"现实世界代码"——而现实世界代码的主体是什么？是最小化影响的补丁、过度防御性编程、用 `.getOrElse(默认值)` 吞掉异常装作没事、能跑就行的权宜之计。这些代码在训练集中的比例远大于精心设计的架构代码。结果就是：**模型的默认倾向（prior）是朝着"最小阻力路径"走的。**

绝对化表述的作用不是描述真理，而是**对抗训练偏差**。它是一个反方向的力，把模型从"默认走最小阻力路径"拉回到"遵循项目的工程标准"。就像矫正视力的眼镜——镜片本身是"歪"的，但戴上之后看到的世界是正的。

这也意味着：**规则文件的绝对化程度应该随模型能力变化而调整。** 如果未来的模型训练数据质量提升，或者 RLHF 对齐策略改变了默认偏好，那些曾经需要用"禁止"来矫正的行为可能变成了模型的自然选择——此时绝对化表述就可以放松为"优先"甚至移除。规则文件不是宪法，它是针对特定模型版本的校准参数。
